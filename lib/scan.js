...
const MIMO_DIR = path.join(HOME, 'AppData', 'Roaming', 'Xiaomi MiMo', 'db');
...
// ---------- 小米 MiMo ----------
// artifacts.db 记录每个会话生成的产物文件（含本地路径），据此归到项目目录；
// evolve-transcript.db 是会话正文表，当前为空，一旦有数据自动接入会话提取

function guessProjectDir(filePath, anchors) {
  let best = '';
  for (const d of anchors) {
    if (filePath.toLowerCase().startsWith(d.toLowerCase() + path.sep) && d.length > best.length) best = d;
  }
  if (best) return best;
  // 兜底：取 桌面/文档/下载 下的一级子目录
  const m = filePath.match(/^([A-Za-z]:\\(?:Users\\[^\\]+\\)?(?:Desktop|Documents|Downloads)\\[^\\]+)/i);
  return m ? m[1] : null;
}

function scanMimoProjects(anchors) {
  const artifactsDb = path.join(MIMO_DIR, 'artifacts.db');
  const transcriptDb = path.join(MIMO_DIR, 'evolve-transcript.db');
  const out = new Map(); // projectDir -> entries
  const push = (dir, entry) => {
    if (!out.has(dir)) out.set(dir, []);
    out.get(dir).push(entry);
  };
  const addDir = dir => { if (dir && !anchors.includes(dir)) anchors.push(dir); };
  const sessionDirs = new Map(); // session_id -> projectDir

  // 产物痕迹：按会话归组，并建立 session -> 项目目录 映射（供会话正文复用）
  try {
    if (fs.existsSync(artifactsDb)) {
      const db = openDb(artifactsDb);
      const rows = db.prepare(
        'SELECT session_id, title, local_path, created_at FROM artifact WHERE local_path IS NOT NULL ORDER BY created_at ASC'
      ).all();
      db.close();
      ...
      for (const [sid, arts] of bySession) {
        const dirs = [...new Set(arts.map(a => guessProjectDir(a.local_path, anchors)).filter(Boolean))];
        if (!dirs.length) continue;
        sessionDirs.set(sid, dirs[0]);
        const when = Math.max(...arts.map(a => toTimestamp(a.created_at)));
        const names = [...new Set(arts.map(a => a.title || path.basename(a.local_path)))];
        const shown = names.slice(0, 15);
        const extra = names.length > 15 ? `\n……等共 ${names.length} 个产物` : '';
        for (const dir of dirs) {
          addDir(dir);
          const lines = [
            'MiMo 会话产物：' + new Date(when).toLocaleDateString('zh-CN'),
            '目录：' + dir,
            '最近活动：' + new Date(when).toLocaleDateString('zh-CN'),
            '',
            '该会话生成了以下产物文件：',
            ...shown.map(n => '- ' + n),
            extra
          ...
          const content = lines.join('\n');
          push(dir, {
            source: 'mimo', scope: 'project', title: lines[0], content,
            updatedAt: when, chars: content.length
          });
        }
      }
      for (const list of out.values()) list.sort((a, b) => b.updatedAt - a.updatedAt);
  ...
  // 会话正文：当前为空表，有数据后经 session_id -> 项目目录 接入
  try {
    if (fs.existsSync(transcriptDb)) {
      const db = openDb(transcriptDb);
      const has = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='evolve_message'").all().length > 0;
      if (has) {
        const total = db.prepare('SELECT COUNT(*) n FROM evolve_message').get().n;
        if (total > 0) {
          const msgs = db.prepare('SELECT session_id, role, text, created_at FROM evolve_message ORDER BY session_id, seq').all();
          const bySession = new Map();
          for (const m of msgs) {
          ...
          for (const [sid, list] of bySession) {
            ...
            if (!dir) continue;
            const firstUser = (list.find(m => m.role === 'user') || {}).text || '';
            const lastAssistant = ([...list].reverse().find(m => m.role === 'assistant' && (m.text || '').trim().length > 50) || {}).text || '';
            if (!firstUser && !lastAssistant) continue;
            const when = toTimestamp(list[list.length - 1].created_at);
            const lines = [(firstUser || 'MiMo 会话').slice(0, 80), '目录：' + dir, '最近活动：' + new Date(when).toLocaleDateString('zh-CN')];
            if (firstUser) lines.push('', '【最初需求】', capText(firstUser, ZCODE_MAX_INPUT));
            if (lastAssistant) lines.push('', '【最终产出】', capText(lastAssistant, ZCODE_MAX_OUTPUT));
            const content = lines.join('\n');
            push(dir, { source: 'mimo', scope: 'project', title: lines[0], content, updatedAt: when, chars: content.length });
          }
        }
      }
      db.close();
...
function discoverProjectDirs(qoderProjects, zcodeProjects, claudeEntriesByDir, codexSessions, mimoProjects) {
  const set = new Set();
  ...
  for (const dir of mimoProjects.keys()) set.add(dir);
  return [...set].filter(d => {
...
function scanAll() {
  ...
  const mimoProjects = scanMimoProjects([...zcodeProjects.keys(), ...qoderProjects.keys(), ...claudeEntriesByDir.keys()]);

  const projectDirs = discoverProjectDirs(qoderProjects, zcodeProjects, claudeEntriesByDir, codexSessions, mimoProjects);
  const projects = projectDirs.map(dir => {
    ...
    return {
      ...
      sources,
      total: sources.qoder.length + sources.claude.length + sources.codex.length + sources.trae.length + sources.zcode.length + sources.mimo.length,
